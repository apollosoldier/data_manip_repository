...
def fit_lgbm(config, dtrain, dval):
    trn_data = lgb.Dataset(
        dtrain[config.features],
        label=dtrain[config.target],
        categorical_feature=config.categoricals,
    )
    val_data = lgb.Dataset(
        dval[config.features],
        label=dval[config.target],
        categorical_feature=config.categoricals,
    )
    clf = lgb.train(
        config.param,
        ...
        verbose_eval=config.vb_eval,
        early_stopping_rounds=config.es,
    )
